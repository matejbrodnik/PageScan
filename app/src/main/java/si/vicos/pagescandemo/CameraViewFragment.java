...
public class CameraViewFragment extends Fragment
    ...
    public static interface CameraProcessor {
     ...
     */
    private ImageReader mImageReader, mPreviewReader;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mOverlay = new PreviewOverlay(getContext());
 ...
 */
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                ImageReader.OnImageAvailableListener captureReaderListener = new ImageReader.OnImageAvailableListener() {
                    @Override
                    public void onImageAvailable(ImageReader reader) {
                        Image image = null;
                        try {
    ...
    /**
     * Opens the camera specified by {@link CameraViewFragment#mCameraId}.
     */
    ...
    public void changeFlash(boolean flash) {
        flashOn = flash;
        try {
            if(flash) {
                mPreviewRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH);
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        switch (rotation) {
            ...
            case Surface.ROTATION_180:
                height = viewWidth;
                width = viewHeight;
                //scaley = 1 / (mPreviewDimension.getHeight() / height);
                //scalex = 1 / (mPreviewDimension.getWidth() / width);
                break;
            case Surface.ROTATION_270:
            case Surface.ROTATION_90:
                break;
        ...
        mOverlayTransform.reset();

        float dx = (viewWidth - mPreviewSize.getWidth()) / 2;
        float dy = (viewHeight - mPreviewSize.getHeight()) / 2 ;
        ...
        Log.d(TAG, String.format("dx = %f dy = %f sx = %f sy = %f", dx, dy, scalex, scaley));
    ...
    class PreviewOverlay extends SurfaceView {
        ...
        protected synchronized void onDraw(Canvas canvas) {
            ...
            mOverlayTransform.invert(cameraTransform);

            canvas.setMatrix(cameraTransform);

            for (Overlay p : overlays) {
                p.draw(canvas, paint);
            }
        ...
        public synchronized void addOverlays(List<Overlay> overlays) {
            this.overlays.addAll(overlays);
            postInvalidate();
        }

        public synchronized void setOverlays(List<Overlay> overlays) {
            this.overlays.clear();
            if (overlays != null)
                this.overlays.addAll(overlays);
            postInvalidate();
